...
Let's define the random seed now for the sake of reproducibility.
...
The behaviors data comprise users' implicit feedback about news items in
the form of history logs and so-called impressions logs. MIND was
...
together with the user's implicit feedback about these items.

```{python}
#| code-summary: "Name of behaviors file"
...
The news data comprise metadata about the news items that may be present
in the behaviors data's history logs and impressions logs, such as
...
some of the news data's named entities.

```{python}
#| code-summary: "Set file name of entity embeddings file"
...
knowledge-graph relations of some of the news data's named entities. We
...
As mentioned previously, we'll partition the training set into two to
reserve some data for validation. Our partitioning scheme will reserve
for validation the training-set data from the most recent day on which
training data were collected; the remainder will be retained for
...
validation set or from the testing set.
...
users to rows established for the interactions matrices, but each will
have its own mapping of features to columns that will be shared with the
corresponding blocks of the other partitions.
...
the features named in `which_user_ft`. Specify
`user' alone here, along with`item' alone later on in `which_item_ft`,
to use LightFM for traditional collaborative filtering.
...
by the total number of items in the user's log so that each non-zero row
of the block sums to one.
...
Next come the item-features. Each partition of the data-set will have an
item-features matrix, which we'll assemble by blocks like we've done
with the user-features. All blocks of a given partition will share the
mapping of items to rows established between items and columns for the
interactions matrices, but each will have its own mapping of features to
columns that will be shared with the corresponding blocks of the other
partitions.
...
the features named in `which_item_ft`.  Note that this isn't an exhaustive
list of the item-features that one might possibly include; consider, for
example, that we could (but won't) incorporate the text of items' titles
and abstracts with some more complex pre-processing.
...
item, each of which will be given a weight of one.
...
The LifghtFM module does not provide access to the loss function, so
earlier stopping will be based on an accuracy-based metric.

```{python}
#| code-fold: show
#| code-summary: "For early-stopping based on normalized discounted cumulative gain"
...
```{python}
#| code-fold: show
#| code-summary: "For early-stopping based on hit-rate at k"
...
```{python}
#| code-fold: show
#| code-summary: "For early-stopping based on mean reciprocal rank"
...
```{python}
#| code-fold: show
#| code-summary: "Function definition enabling collection of metrics"
...
```{python}
#| code-fold: show
#| code-summary: "Define evaluation metric: Normalized Discounted Cummulative Gain"
...
```{python}
#| code-fold: show
#| code-summary: "Define evaluation metric: Hit rate at k"
...
```{python}
#| code-fold: show
#| code-summary: "Define evaluation metric: Reciprocal rank"
...
If early-stopping is disabled then we'll merely train a recommender with
...
the evaluation metrics in `stats` (see below) from the recommender's
predictions for the testing set's user-item pairs.

```{python}
#| code-fold: show
#| code-summary: "Evaluate model on test data"
#| 
# Set `want_testing' to `True' to evaluate recommender on testing set.
want_testing = False
te = data.get("te")
if want_testing and (data.get("te") is not None):
    stats = {
        "mean HR@k":            mean_hrk,
        "mean NDCG":            mean_ndcg,
    ...
    yhat = model.predict(
        user_ids      = te["ui"].row,
        item_ids      = te["ui"].col,
        user_features = te["uft"],
        item_features = te["ift"],
        num_threads   = n_avail_cpu()
    )
    # The implementations of the evaluation metrics in `stats' each take two
    # LIL-format sparse user-item matrices as input: One assumed to provide
    # labels (`cond') and, the other (`pred'), predictions.
    cond = sp.coo_matrix(te["ui"])
    pred = sp.coo_matrix((yhat, (cond.row, cond.col)), shape=cond.shape)
    # for all users
    _cond = cond.tolil()
    _pred = pred.tolil()
    ...
    I = list(set(te["ui"].row) & set(data["tr_full"]["ui"].row))
    _cond = cond.tolil()[I]
    _pred = pred.tolil()[I]
