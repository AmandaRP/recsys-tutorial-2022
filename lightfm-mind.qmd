...
As mentioned previously, we'll partition the training set in two to
...
For each subset of the data-set (training, validation, testing) we'll
require a sparse matrix in coordinate-list format of user-item
interactions whose rows correspond to users, whose columns correspond to
items and whose entries specify the users' implicit feedback about the
...
```{python}
#| code-fold: show
#| code-summary: "Evaluate simple baseline"
cond = sp.coo_matrix(data["te"]["ui"])
pred = sp.coo_matrix((yhat, (cond.row, cond.col)), shape=cond.shape)
_cond = cond.tolil()
_pred = pred.tolil()
